...
export class GrpcGenerator {
  ...
  public execute(): void {
    this.getProtoPaths().forEach((protoPath: string) => {
      ...
      this.protos[protoName] = this.loadProto(protoName);
  ...
  /**
   * This method receive a proto file name and
   * load that proto using the official @grpc/grpc-js library.
   *
   * @param protoName
   */
  public loadProto(protoName: string): GrpcObject {
    const packageDefinition = loadSync(protoName)
